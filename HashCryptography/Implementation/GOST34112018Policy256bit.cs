...
namespace HashCryptography.Implementation
{
    public class GOST34112018Policy256bit
        : IHashPolicy
    {
        public byte[] GetHash(byte[] message)
        {
            ...
            // Stage 2
            uint len = Convert.ToUInt32(message.Length * 8);
            uint iteration = 0;
            while (len >= 512)
            {
                ++iteration;
                byte[] tempMes = new byte[64];
                Array.Copy(message, message.Length - iteration * 64, tempMes, 0, 64);
                h = Gn(N, h, tempMes);
                N = AddModulo512(N, BitConverter.GetBytes(512).Reverse().ToArray());
            ...
            Array.Copy(message, 0, m, 64 - (message.Length - iteration * 64), message.Length - iteration * 64);
            m[64 - (message.Length - iteration * 64) - 1] = 0x01;
            h = Gn(N, h, m);
            N = AddModulo512(N, BitConverter.GetBytes((message.Length - iteration * 64) * 8).Reverse().ToArray());
        ...
        private byte[] L(byte[] state)
        {
            ...
            for (int i = 0; i < 8; ++i)
            {
                ...
                for (int j = 0; j < 8; ++j) tempArray[j] = state[i * 8 + (8 - j - 1)];
                BitArray tempBits = new BitArray(tempArray);
                ulong t = 0;
                for (int j = 63; j >= 0; --j)
                {
                    if (tempBits[j])
        ...
        private byte[] Gn(byte[] N, byte[] h, byte[] m)
        {
        ...
        private byte[] AddModulo512(byte[] a, byte[] b)
        {
            ...
            for (int i = 63, t = 0; i >= 0; --i)
            {
                t = tempA[i] + tempB[i] + (t >> 8);
                temp[i] = Convert.ToByte(t & 0xFF);
            }
            return temp;
        }

        // Initialization vector (00000000^64 for 512 bit)
        private byte[] m_IV =
        {
        ...
        // S-transform constats (256 elements)
        private byte[] m_Pi =
        {
        ...
        // P-transform constats (64 elements)
        private byte[] m_Tau =
        {
        ...
        // L-transform constats (64 elements)
        private ulong[] m_A =
        {
